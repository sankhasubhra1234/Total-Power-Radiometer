...
me.each(function(){let audioUrl=jQuery('audio',jQuery(this)).attr('src')
if(!audioUrl)
return
...
me_pl.each(function(){var el=jQuery(this)
...
albumInfo.find('span').each(function(){var meta=jQuery(this)
...
var counter=0
me_pl_elements.each(function(){counter++
...
if(counter==1){pl_el.addClass('sel')
jQuery('#'+id).mb_miniPlayer_changeFile({mp3:elementsUrl},title+author)}
if(miniAudioPlayer_defaults.downloadable){var dwnload=pl_player[0].player.createDownload(elementsUrl,title)
pl_el.append(dwnload)
jQuery('.mbMiniPlayer .map_download',pl).remove()}
pl_el.on('click',function(){jQuery('.sel',pl).removeClass('sel')
